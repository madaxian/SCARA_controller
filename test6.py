...
class Demo(QMainWindow, Ui_MainWindow):
    ...
    def tabCJ_func(self, index):
        if (index == 0):
            #####################下面为坐标界面数值设定：
            self.doubleSpinBox_x.setValue(self.doubleSpinBox_j1.value() + self.doubleSpinBox_j2.value())
            #####################
        else:
            #####################下面为关节界面数值设定：
            self.doubleSpinBox_j1.setValue(self.doubleSpinBox_x.value() + self.doubleSpinBox_y.value())
            #####################
